...
<div class="zhibo11">
    <div class="broadcast" id="myimg" style="height:auto;">
        <img src="//admin.mumway.com/<?php echo ($pd["vpic"]); ?>" onerror="imgerror()" style="height:320px;">
...
<div class="zhibo_tab">
    <ul class="synopsis">
        ...
        <li style="width:50%;">章节（可单集购买）</li>
        <!-- 
        <li style="width:33.3%;">问答</li>
    ...
    <div class="zhibo_tab1 current">
        <div class="broadcast_p2" style="padding-bottom:15px;">
            ...
            <P class="broadcast_s2">已有<span><?php echo ($pd["see_mum"]); ?></span>个同学学过</P>
        ...
        <div class="zhibo_tab_d1 " style="color: red;">
        	<p>
        	<?php echo ($pd["price"]); ?>元/专辑（
	        <?php switch($pd["id"]): ; case "38": ?>20集）9.9元/集<?php break;?>
	        <?php case "39": ?>42集）8.8元/集<?php break;?>
	        <?php default: ?>22集）9.9元/集<?php endswitch;?>
        	</p>
        </div>
    ...
    -->
    <div class="zhibo_tab1">
    	<?php if(($pd["type"] == '课程')): ; if(is_array($pd["classList"])): foreach($pd["classList"] as $key=>$tt): ?><div class="list">
            	<P class="list_p1"><?php echo ($tt["name"]); ?></P>
                <ul class="list_ul">
                <?php if(is_array($tt["video"])): foreach($tt["video"] as $key=>$t): ?><li>
	                <span class="list_ul_a1"><?php echo ($t["title"]); ?></span>
	                <span class="list_ul_a22"><?php if(empty($t["oid"])): ; endif; ?>
                	<?php if(($t["money"] != 0)): ?><a href="__URL__/zhifu/vid/<?php echo ($t["id"]); ?>/openid/<?php echo ($openid); ?>"><?php echo (floatval($t["money"])); ?>元购</a> | <a href="__URL__/quzhengxinxi/id/<?php echo ($pd["id"]); ?>/vid/<?php echo ($t["id"]); ?>/openid/<?php echo ($openid); ?>">播放</a>
					<?php else: ?>
						<a href="__URL__/quzhengxinxi/id/<?php echo ($pd["id"]); ?>/vid/<?php echo ($t["id"]); ?>/openid/<?php echo ($openid); ?>">免费播放</a><?php endif; ?>
	                </span>
		...
		<?php else: ?>
			<div class="list">
                <ul class="list_ul">
					<?php if(is_array($pd["classList"])): foreach($pd["classList"] as $key=>$t): ?><li>
                		<span class="list_ul_a1"><?php echo ($t["title"]); ?></span>
	                	<span class="list_ul_a22"><?php if(empty($t["oid"])): ; endif; ?>
	                	<?php if(($t["money"] != 0)): ?><a href="__URL__/zhifu/vid/<?php echo ($t["id"]); ?>/openid/<?php echo ($openid); ?>"><?php echo (floatval($t["money"])); ?>元购</a> | <a href="__URL__/quzhengxinxi/id/<?php echo ($pd["id"]); ?>/vid/<?php echo ($t["id"]); ?>/openid/<?php echo ($openid); ?>">播放</a>
						<?php else: ?>
...
<script type="text/javascript">
	...
	$(function(){
		...
		if(BOUGHT){
			...
			var video = '<?php echo ($pd["vurl"]); ?>';
			if(video.indexOf("live.ceecloud.cn") > -1){
				//云见直播
				document.getElementById("myvidoe").innerHTML="<iframe style='margin-left:-8px; margin-top:-8px;' width='656px' height='366px' id='iframeZhibo' name='iframeZhibo' src='<?php echo ($pd["vurl"]); ?>' frameborder='0' scrolling='no' webkitallowfullscreen='' mozallowfullscreen='' allowfullscreen=''></iframe>";
				...
				//资源播放
				document.getElementById("myvidoe").innerHTML='<video src="<?php echo ($pd["vurl"]); ?>" controls="controls" height="320px" width="640px">您的浏览器不支持播放</video>';
			}
		}
