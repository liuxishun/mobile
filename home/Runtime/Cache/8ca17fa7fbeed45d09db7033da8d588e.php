...
<script type="text/javascript">
	...
	function pay(){
		if(BOUGHT){
			...
			//添加一条用户学习轨迹视频的记录
			var ul = 'http://m.mumway.com:8088/JYYS/api/user/saveUserVideo?USER_ID=<?php echo ($openid); ?>&VIDEO_ID=<?php echo ($pd["vid"]); ?>';
...
<div class="zhibo11">
    <div class="broadcast" id="myimg" style="height:auto;">
        <img src="http://m.mumway.com:8088/JYYS/<?php echo ($pd["pic"]); ?>" style="height:320px;">
...
<div class="zhibo_tab">
    ...
    <div class="zhibo_tab1">
        <?php if(is_array($pd["classList"])): foreach($pd["classList"] as $key=>$tt): ?><div class="list">
            	<P class="list_p1"><?php echo ($tt["title"]); ?><!-- <a href="#">考试</a> --></P>
                <ul class="list_ul">
                <?php if(is_array($tt["video"])): foreach($tt["video"] as $key=>$t): ?><li><a class="list_ul_a1" href="javascript:;"><?php echo ($t["title"]); ?></a><a class="list_ul_a2" href="__URL__/jinxiu1/id/<?php echo ($pd["id"]); ?>/vid/<?php echo ($t["id"]); ?>/openid/<?php echo ($openid); ?>" >播放</a></li><?php endforeach; endif; ?>
                </ul>
...
<footer class="broadcast_foot">
    <P class="broadcast_foot_p"><a href="tel:15101682551">课程咨询</a><span><i class="i1"><?php echo ($pd['jine']+300).'元'; ?></i><i class="i2"><?php echo ($pd["jine"]); ?></i></span><input type="submit" onclick="javascript:location.href='__URL__/zhifu?classid=<?php echo ($pd["id"]); ?>&openid=<?php echo ($openid); ?>'" value="立即抢购"></P>
...
<script type="text/javascript">
	...
	function xiao(){
		var url = '__URL__/getpinglun/urlid/<?php echo ($pd["id"]); ?>/openid/<?php echo ($openid); ?>';
		$.get(url,function(dd){
