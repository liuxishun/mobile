...
// 		$title = $jine.'*'.$_POST['name'].'*'.$_POST['did'].'-'.$_POST['zhi'];
		//$title = '1*测试*345-zhibo*https://www.baidu.com/';
		//触发微信返回code码
		$url = $jsApi->createOauthUrlForCode('http://m.mumway.com/demo2/pingfu2.php',$title);
		Header("Location: $url"); 
	}else
	{
		//获取code码，以获取openid
	...
	$search = mysql_query("select phone,name from user_weixin where openid = '{$openid}'",$conn);
	$search = mysql_fetch_row($search);
	if(!$search[0]){
		$arr[3] = "http://m.mumway.com/wx_web/reglogin.php?openid={$openid}";
	}
	mysql_close();
