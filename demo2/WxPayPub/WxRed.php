...
CLASS WxRed {
  ...
  private $mch_id = "1287611701";//商户ID写死
  private $wxappid = "wxbfaa1d17ea7b5920";//微信公众号，写死
  private $apikey = "mumwayxzw2016888mumway8889999lbq";//pay的秘钥值
  ...
  //证书
  private $apiclient_cert = 'WxPayPub/cacert/apiclient_cert.pem'; //"/apiclient_cert.pem";
  private $apiclient_key = 'WxPayPub/cacert/apiclient_key.pem';//"/apiclient_key.pem";
  ...
  public function send($url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack",$total_num = 1){
    ...
    */    
    ...
    $data = curl_exec($ch);
    file_put_contents("hbrsxml.txt",$data);
    if($data){
      curl_close($ch);  
      ...
      $this->error = 'curl_errno:'.curl_errno($ch);
      curl_close($ch);
      return false;
    }
  ...
  private function gen_Sign(){
    ...
    file_put_contents("hbpaxml.txt",$param);
  ...
  public function genXMLParam(){
    ...
    if($this->nick_name) $xml .= "<nick_name><![CDATA[".$this->nick_name."]]></nick_name>";
