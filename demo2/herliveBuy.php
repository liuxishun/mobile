...
*/
	...
	if (!isset($_GET['code']))
	{
		...
		$title = $_GET['name'].'*'.$_GET['phone'].'*'.$_GET['id'];
	...
	}else
	{
		...
		if($search){
			...
			if($order){
				$sql = "update newadmin.mumway_herlive_order set orderno = '$out_trade_no', updatetime = $timeStamp where id = $order[0]";
				mysql_query($sql,$conn);
				$did = $order[0];
			}else{
				$sql = "INSERT newadmin.mumway_herlive_order (name,phone,amount,orderno,openid,herlive_course_id,herlive_course_name,createtime,updatetime) VALUES ('$arr[0]','$arr[1]',$search[3],'$out_trade_no','$openid',$search[0],'$search[1]',$timeStamp,$timeStamp)";
				mysql_query($sql,$conn);
				$did =  mysql_insert_id();
			}
			...
			WeixinJSBridge.invoke(
				...
				function(res){
					...
					if(res.err_msg == 'get_brand_wcpay_request:ok') {
						window.location.href="http://admin.mumway.com/Herlive/index.html";
					}else {
						alert('支付失败！');
