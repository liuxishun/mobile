...
<div class="right">
      <div class="w810 l">
		...
		<div class="web_tit">
			...
			用户名：<input type="text" id="username" value="<?php echo ($para["username"]); ?>" size="8" >
			用户组：<select id="role_id" style="font-size:12px" <?php if(!C('MY_AUTH_ON')){ ?>style="display:none;"<?php } ?>>
				<option value="">--请选择--</option>
				<?php if(is_array($rlist)): foreach($rlist as $key=>$val): ?><option value="<?php echo ($val["id"]); ?>" <?php if($val['id']==$para['role_id'])echo 'selected'; ?>><?php echo ($val["name"]); ?></option><?php endforeach; endif; ?>
			 </select>
        ...
        <table  class="list_table">
          ...
          <?php if(is_array($list)): foreach($list as $key=>$val): ?><tr>
					...
					<?php if(C('MY_AUTH_ON')){ ?><td><?php echo ($val["role"]["name"]); ?></td><?php } ?>
					<td><?php echo date("Y-m-d H:i:s", $val['addtime']);?></td>
					<td><?php if($val['lastlogin'] != 0): ; echo date("Y-m-d H:i:s",$val['lastlogin']);; else: ?>暂时没登陆过<?php endif; ?></td>
					<td><?php echo ($val["login_num"]); ?></td>
					<td>
						<?php if(C('MY_AUTH_ON')){ ?>
						<a href="__APP__/rbac/user_role_edit/id/<?php echo ($val["id"]); ?>/username/<?php echo base64_encode($val['username']); ?>/listurl/<?php echo base64_encode(urlencode(__SELF__)); ?>">配置权限组</a> |
						<?php } ?>
