...
<div class="right">
	<div class="w810 l">
		<div class="add_tit cl">
			<span class="l">位置：<a href="__APP__/Index/right">管理平台</a><b>管理员编辑</b></span>
		</div>
			...
			<form method="post" action="__SELF__" id="form" name="form">
				<table class="add_table">
					...
					<tr>
						<th >新密码：</th>
						<td>
							<input name="xPassword" type="password" class="add_input" size="20" />
							*
						</td>
					</tr>
					<tr>
						...
						<td><select name="diid" class="add_input">
							<?php if(is_array($dian)): foreach($dian as $key=>$it): ?><option value="<?php echo ($it["id"]); ?>" <?php if($info['diid']==$it['id'])echo 'selected'; ?>><?php echo ($it["title"]); ?></option><?php endforeach; endif; ?>
							</select>
