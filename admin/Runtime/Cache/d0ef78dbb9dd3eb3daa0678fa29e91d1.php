...
<div class="right">
 ...
 $mum_types = D('admin://Member')->mum_type; ?>
				<?php if(is_array($mum_types)): foreach($mum_types as $key=>$val): ?><option value="<?php echo ($val); ?>" <?php if($val==$para['mum_type']) echo 'selected'; ?>><?php echo ($val); ?></option><?php endforeach; endif; ?>
			 </select>
        ...
        <table  class="list_table">
          ...
          <?php if(is_array($list)): foreach($list as $key=>$val): ?><tr>
					...
					<td class="td_left"><?php echo ($val["title"]); ?></td>
					<td><?php echo ($val["cus_id"]); ?></td>
					<td>
						<?php echo ($val["mum_type"]); ?>
					</td>
					<td>
						<?php echo ($val["price"]); ?></del>
					</td>
                    <td>
                    	<?php if ($val['buy_mode']==1): ?>
                    	限时免费<?php echo ($val["free_week"]); ?>周
                    	<?php else: ?>
                    	免费
                    	<?php endif ?>
                    </td>
