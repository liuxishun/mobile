...
<div class="right">
      <div class="w810 l">
		<div class="add_tit cl">
			<span class="l">位置：<a href="__APP__/Index/right">管理平台</a><b>服务列表</b></span>
		</div>
		<div class="web_tit">
        ...
        <table  class="list_table">
          ...
          <?php if(is_array($list)): foreach($list as $key=>$val): ?><tr>
					...
					<td>
						<?php if ($val['hwtype_id']): ?>
							<?php echo M('hwtype')->where(array('id'=>$val['hwtype_id']))->getField('name'); ?>
						<?php endif ?>
					</td>
					<td>
						<?php echo ($val["price_desc"]); ?>
						<del><?php echo ($val["market_price"]); ?></del>
					</td>
                    <td><?php echo ($val["follow_num"]); ?>&nbsp;/&nbsp;<?php echo ($val["review_num"]); ?></td>
					<td>
