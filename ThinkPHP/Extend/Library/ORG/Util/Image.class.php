...
class Image {
     ...
     * @param string $maxHeight  高度
     * @param boolean $interlace 启用隔行扫描
     * //# $keep_w_h 如果小于指定尺寸，保持长宽比
     * @return void
     */
    static function thumb2($image, $thumbname, $type='', $maxWidth=200, $maxHeight=50, $interlace=true, $keep_w_h=false) {
        // 获取原图信息
        $info = Image::getImageInfo($image);
        if ($info !== false) {
            $srcWidth = $info['width'];
            ...
            //#
            if($keep_w_h && ($maxWidth>$cutWidth || $maxHeight>$cutHeight)){
                $maxWidth = $cutWidth;
                $maxHeight = $cutHeight;
            }
